...
FROM (
         SELECT t1.*, IFNULL(p.`课时`, w.`课时`) AS 课时
              ...
              , IFNULL(IF(t1.`订单类型` = "售卡"
                              AND t1.`订单来源` = "微信小程序", IF(t1.`合计收款` = 319, 15, IF(t1.`合计收款` >= 599
                                                                                                   AND c1.`成为会员时间` IS NOT NULL, 30, 0)), 0), 0) AS 开卡奖金
         FROM (
                  SELECT *, IF(`商品优惠` = "卡项权益"
                                   OR (`商品名称` LIKE '%（CP）%'
                          AND `合计收款` != 0)
             ...
             WHERE p.`开卡率` = c.`开卡率`
                  ...
                  LEFT JOIN customer c1
                            ON DATE_FORMAT(c1.`成为会员时间`, '%Y-%m-%d') = DATE_FORMAT(t1.`下单日期`, '%Y-%m-%d')
                                AND c1.`手机号` = t1.`下单人手机号`
