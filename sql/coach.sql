SELECT `技师`, sum(`有效课时费`) 课时费 , sum(`开卡奖金`) 开卡奖金,  SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) 销售总额,

       case
           WHEN 10000 > SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) THEN 0
           WHEN 10000 <= SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) AND SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) < 20000 THEN SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) * 0.03
           WHEN 20000 <= SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) AND SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) < 40000 THEN SUM(IF(`订单类型` = "售卡", IFNULL(`合计收款`, 0), 0)) * 0.04
