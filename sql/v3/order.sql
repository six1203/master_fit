...
FROM
    (
            ...
            IFNULL(
                    ...
                    (
                            ...
                            (
                                ...
                                (
                                            t1.`课时新老客标识` = "老客",
                                            IFNULL( p.`课时`, w.`课时` ) * IFNULL( p.`系数`, c2.`系数` ) * t1.数量,
                                            p.`课时费`
                                )
                                ),
                            p.`固定课时费`
                    ...
                    (
                                ...
                                AND t1.退款状态 != '退款成功',
                                    ...
                                    (
                                                ...
                                                15,
                                                IF
                                                ( t1.`合计收款` >= 599 AND c1.`成为会员时间` IS NOT NULL AND TIMESTAMPDIFF(DAY, t1.`下单日期`, c1.`首次消费时间` ) <= 2, 30, 0 )),
                                    0
                    ),
                    0
            ) 开卡奖金
        FROM
            (
                SELECT
                    *,
                    IF
                    ( `商品优惠` = "卡项权益" OR ( `商品名称` LIKE '%（CP）%' AND `合计收款` != 0 ) OR ( `商品名称` IN ( SELECT `商品名称` FROM old_customer_whitelist )), "老客", "新客" ) "课时新老客标识"
                FROM
                    `order`
            ) t1
                LEFT JOIN (
        ...
        LEFT JOIN (
        SELECT
            o.序号, if(o.新老客 = "散客" OR (c.`首次消费时间` is NOT NULL AND TIMESTAMPDIFF(DAY, o.`下单日期`, c.`首次消费时间` ) <= 2), "新客", "老客") 开卡新老客标识
        FROM `order` o
                 LEFT JOIN customer c on c.`手机号` = o.`下单人手机号`
    ) tmp2 on tmp.序号 =  tmp2.序号